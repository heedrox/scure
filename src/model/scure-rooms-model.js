...
const getLockedIds = unlocked => (destination) => {
  if (destination.isLockedDestination && !isUnlocked(unlocked, destination)) return destination.roomId;
  return null;
};
...
const roomById = id => r => (r.isLockedDestination && r.roomId === id) || (!r.isLockedDestination && r === id);
...
class ScureRoomsModel {
  ...
  getDestinationLockedSentence(destinationName, fromRoomId) {
    const room = this.getRoomByName(destinationName);
    const theLockedDestination = this.map[fromRoomId].find(r => r.isLockedDestination && r.roomId === room.id);
